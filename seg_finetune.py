...
class SegFinetuneDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if mask.ndim == 3:      # someone handed us an RGB mask by mistake
            raise ValueError(
                f"Mask {mask_path.name} is {mask.ndim}-channel; label masks must "
                f"be single-channel (pixel value = class id). Convert it first.")
        h, w = mask.shape
        assert (w, h) == img.size, (
        ...
        labels = torch.from_numpy(np.asarray(mask_pil)).long()     # [H,W] class ids
