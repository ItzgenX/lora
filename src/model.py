...
class ModelBase(ABC, nn.Module):
    ...
    def __init__(
        ...
        if tiny_vae:
            ...
            # For base checkpoints published WITHOUT a baked-in VAE (e.g. the
            # "noVAE" variant of Realistic Vision and similar community
            # finetunes) -- those checkpoints EXPECT a separate VAE to be
            # paired in, most commonly vae-ft-mse-840000-ema-pruned. Without
            # this, `self.vae = self.pipe.vae` (below) would silently load
            # whatever placeholder/default VAE from_pretrained finds (or
            # error), producing broken latents with no clear signal why.
            # Mutually exclusive with tiny_vae (elif, not a second `if`).
...
class SD15(ModelBase):
    ...
    def sample_custom(
        ...
        **kwargs,
        ...
        # behaviour) UNLESS the caller passes them explicitly. This class had
        # NO way at all to produce non-square output before this -- the values
        # were computed unconditionally from unet.config.sample_size, so ANY
        # kwarg override was silently ignored. segformer_training.py's
        # checkpoint-monitoring grids call this method directly (NOT
        # sample_easy, which already threads height/width through **kwargs to
        # the underlying pipeline) -- without this fix, every monitoring image
