...
ORIGINAL CLASSES (ImageFolderDataset / ZipDataset / ImageDataModule): the
repo's original dataset classes, kept unmodified. They load only
(image, caption) — original training computes depth LIVE from the image, so
no depth field is needed. Used by the original train.py entrypoint.

(The segmentation JSON-manifest dataset lives in src/data/local_seg.py.)
"""
import os
